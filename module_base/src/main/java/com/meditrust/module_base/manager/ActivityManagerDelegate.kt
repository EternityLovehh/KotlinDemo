...
class ActivityManagerDelegate {
    ...
    fun topActivity(): Activity? {
        checkWeakReference()
        return if (mActivityStack != null && !mActivityStack!!.isEmpty()) {
            mActivityStack!!.firstElement().get()
        } else null
    }
    ...
    companion object {
        @Volatile
        private var sInstance: ActivityManagerDelegate? = null

        /**
         * 单例
         * @return 返回AppManager的单例
         */
        val instance: ActivityManagerDelegate?
            get() {
                if (sInstance == null) {
                    sInstance = ActivityManagerDelegate()
                }
                return sInstance
            }
    }
